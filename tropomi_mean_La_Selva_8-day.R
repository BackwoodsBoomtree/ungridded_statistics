...
input_files <- list.files("G:/TROPOMI/esa/extracted/ebf/La_Selva", pattern = "*.nc", full.names = TRUE, recursive = TRUE)
out_name    <- "La_Selva_TROPOSIF_L2B_2019-2021_sifd_mean_timeseries_monthly"
out_dir     <- "G:/Costa_Rica/La_Selva/tropomi/"
years       <- c(2018:2021)
time        <- "month"
  ...
  if (time == "8-day") {
    ...
    for (i in 1:46) {
      ...
      for (j in 1:length(sub_dates)) {
        ...
        check_file <- input_files[grepl(sub_dates[j], input_files)]
